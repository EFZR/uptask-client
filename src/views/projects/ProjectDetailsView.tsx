...
import { getProjectByWithTask } from "@/services/Projects";
...
export default function ProjectDetailsView() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => getProjectByWithTask(projectId),
    retry: false,
  });

  const isManager = useMemo(() => data?.manager === user?._id, [user, data]);
  ...
  if (data && user)
    return (
      <>
        ...
        <TaskList tasks={data.tasks} isManager={isManager} />
        <AddTaskModal />
        <EditTaskData />
        <TaskModalDetails />
      </>
